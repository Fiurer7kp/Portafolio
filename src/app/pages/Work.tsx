    ...
    >
      ...
      <div className="grid md:grid-cols-2 gap-10 items-center">
        <div key={current} style={{ animation: 'fadeInLeft 0.4s ease' }}>
          <span className="text-6xl font-bold block mb-4 leading-none"
          ...
          <div className="flex flex-wrap gap-2 mb-8">
            {project.tags.map((tag) => (
              <span key={tag} className="text-xs font-semibold px-3 py-1 rounded-full"
                style={{ background: 'var(--bg-soft)', color: 'var(--accent)', border: '1px solid var(--accent-light)', fontFamily: 'DM Sans, sans-serif' }}>
                {tag}
              </span>
            ))}
          </div>
        ...
        <div className="relative">
          <div className="rounded-2xl overflow-hidden"
            style={{ boxShadow: '0 20px 60px var(--accent-glow)', border: '1px solid var(--border)' }}>
      ...
      <div className="flex justify-center gap-3 mt-10">
        {projects.map((_, i) => (
          <button key={i} onClick={() => setCurrent(i)}
            ...
            style={{ width: i === current ? '32px' : '10px', height: '10px', background: i === current ? 'var(--accent)' : 'var(--border)' }} />
        ))}
      </div>
